...
import math
from tools.process_song import process_song_url
...
class ToolRegistry:
    def __init__(self, lyrics_path: Path, vocabulary_path: Path):
        ...
        self.tools = {
            ...
            'process_song': partial(process_song_url, lyrics_path=self.lyrics_path, vocabulary_path=self.vocabulary_path),
...
class SongLyricsAgent:
    ...
    def parse_llm_action(self, content: str) -> Optional[tuple[str, Dict[str, Any]]]:
        ...
        # Special handling for complex types (lists, dicts)
        if "vocabulary=" in args_str and "vocabulary" not in args:
            # Extract vocabulary list
            vocab_match = re.search(r'vocabulary=(\[.*?\])', args_str)
            if vocab_match:
                try:
                    vocab_str = vocab_match.group(1).replace("'", '"')  # Convert to JSON
                    args["vocabulary"] = json.loads(vocab_str)
                except:
                    logger.warning(f"Failed to parse vocabulary: {vocab_match.group(1)}")
                    args["vocabulary"] = []
    ...
    async def execute_tool(self, tool_name: str, args: Dict[str, Any]) -> Any:
        ...
        # Add special handling for save_results
        if tool_name == "save_results":
            required = ["song_id", "lyrics", "vocabulary"]
            missing = [param for param in required if param not in args]
            if missing:
                error = f"Missing required parameters for save_results: {', '.join(missing)}"
                logger.error(error)
                return {"error": error, "help": "save_results requires song_id, lyrics, and vocabulary"}
        ...
        try:
            ...
            error = f"Tool Failed: {tool_name} - {e}"
            logger.error(error)
            return {"error": error}
    ...
    def _get_llm_response(self, conversation):
        ...
        if self.stream_llm:
            ...
            full_response = ""
            logger.info("Streaming tokens using model: mistral")
            ...
            try:
                logger.info("Using model in non-streaming mode: mistral")
                ...
                try:
                    ...
                    if not action:
                        if 'FINISHED' in response['content']:
                            ...
                            conversation.append({
                                "role": "system", 
                                "content": "I didn't find a valid tool call. Please use one of these tools in the exact format 'Tool: tool_name(arg1=\"value\")': search_web_serp, get_page_content, extract_vocabulary, generate_song_id, save_results"
                            })
                            continue
                except Exception as e:
